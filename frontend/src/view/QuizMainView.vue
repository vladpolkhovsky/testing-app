  ...
  <div class="flex flex-col shrink items-center p-3 w-full xl:w-6xl mx-auto">
    <LiquidGlass class="w-full xm:w-3xl p-0 flex flex-col gap-8">
      <div class="w-full max-w-6xl mx-auto">
        ...
        <LiquidGlass class="h-fit p-6" style="z-index: 10;">
          <div class="flex flex-col items-center lg:flex-row justify-between lg:items-center gap-4">
            ...
            <div class="flex flex-wrap items-center gap-3">
              ...
              <!-- QR Code Button -->
              <div class="relative">
                <button @mouseenter="QRCodeStateChanger(true)"
        ...
        <!-- Main Content Area -->
        <LiquidGlass class="h-full relative min-h-[500px] p-2" style="z-index: 0;">
...
const questionComponentRef = useTemplateRef<InstanceType<typeof QuestionComponent>>("questionComponentRef")
const timerComponentRef = useTemplateRef<InstanceType<typeof Timer>>("timerComponentRef")
...
onMounted(() => {
  ...
  socketService.setOnStartQuizQuestionCallback(message => {
    ...
    if (timerComponentRef.value) {
      timerComponentRef.value.updateTimer(duration);
    }
  });
...
const qrCodeState = reactive<QrCodeState>({
  show: false,
  location: window.location + "/p" as string,
});
