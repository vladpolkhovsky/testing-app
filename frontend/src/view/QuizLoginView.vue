  ...
  <div class="w-full max-w-2xl mx-auto">
    <LiquidGlass class="bg-white/70 w-full max-w-sm sm:max-w-md mx-auto p-6 sm:p-8 rounded-2xl">
      ...
      <div class="text-center mb-6 sm:mb-8">
        <div class="inline-flex items-center justify-center w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-gradient-to-r from-blue-500 to-purple-500 mb-4">
      ...
      <!-- Login Form -->
      <form @submit.prevent="onLogin()" class="space-y-4 sm:space-y-5">
        <!-- Username Input -->
        <div class="space-y-2">
          ...
          <p class="text-xs text-gray-500 mt-1">
            Это имя будет отображаться в таблице результатов
          </p>
        </div>
        ...
        >
          ...
          <span>
            {{ isLoading ? 'Вход...' : 'Присоединиться к игре' }}
          </span>
        </button>

        <!-- Room Info (if redirected) -->
        <div
            v-if="route?.query?.hardRedirectTo"
            class="mt-4 p-3 rounded-lg bg-blue-50/50 border border-blue-200"
        >
          <p class="text-xs sm:text-sm text-blue-700 text-center">
            Вы присоединяетесь к активной игре
          </p>
        </div>
      </form>
    </LiquidGlass>
